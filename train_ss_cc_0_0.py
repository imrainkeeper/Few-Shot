...
import cv2
import time
from models.net_ss_cc_0_net0 import net
from utils.utils_ss_cc import save_checkpoint
from data.data_loader_ss_cc_0 import ImageDataset
...
args.best_epoch = -1
checkpoint_save_dir = '/home/rainkeeper/Projects/PycharmProjects/rain24/checkpoints_rain24_ss_cc_0_0_try0'
...
def main():
    ...
    train_image_dir = '/home/rainkeeper/Projects/Datasets/few_shot/self_supervised_data' + str(args.dataset_id) + '/Train_Image/crop_count'
    val_image_dir = '/home/rainkeeper/Projects/Datasets/few_shot/self_supervised_data' + str(args.dataset_id) + '/Test_Image/crop_count'
...
def train(train_image_dir, model, criterion, optimizer, epoch):
    ...
    for i, (imgs, img_name) in enumerate(train_loader):
        ...
        crowd_counts = model(images)      # 定义: 人越小，distance越小

        x1 = torch.empty(10).to(args.device)
        x2 = torch.empty(10).to(args.device)
        y = torch.empty(10).to(args.device)
        l1 = torch.empty(10).to(args.device)
        l2 = torch.empty(10).to(args.device)

        index = 0
        for m in range(4):
            for n in range(m + 1, 5, 1):
                x1[index] = crowd_counts[m]
                x2[index] = crowd_counts[n]
    ...
    accuracy = count / (10 * len(train_loader.dataset))
...
def val(val_image_dir, model, epoch):
    ...
    with torch.no_grad():
        for i, (imgs, img_name) in enumerate(val_loader):
            ...
            crowd_counts = model(images)

            crowd_counts = [crowd_count.data.cpu().numpy() for crowd_count in crowd_counts]
            labels = [label.data.cpu().numpy() for label in labels]

            for m in range(5):
                for n in range(m + 1, 5, 1):
                    if (labels[m] - labels[n]) * (crowd_counts[m] - crowd_counts[n]) > 0:
                        count += 1

            if i % 100 == 0:
                for m in range(len(crowd_counts)):
                    print('distances: %.6f, labels: %d' % (crowd_counts[m], labels[m]))
                print('\n')
                for m in range(len(crowd_counts)):
                    print('distances: %.6f, labels: %d' % (crowd_counts[m], labels[m]))
                print('\n', file=terminal_file)

    accuracy = count / (10 * len(val_loader.dataset))
