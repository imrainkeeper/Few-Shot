#################################################
# rain24中的train_ss_dcc_0_1.py和rain24中的train_ss_dcc_0_0.py相比，distance的逻辑改变了。
#################################################
...
import cv2
import time
from models.net_ss_dcc_0_net0 import net
...
args.best_epoch = -1
checkpoint_save_dir = '/home/rainkeeper/Projects/PycharmProjects/rain24/checkpoints_rain24_ss_dcc_0_1_try0'
...
def train(train_image_dir_distance, train_image_dir_crowd_count, model, criterion, optimizer, epoch):
    ...
    for i, (imgs, img_name) in enumerate(train_loader):
        ...
        distance_and_count = model(images)      # 定义: 人越小，distance越大，label越小
        ...
        y_d = [y_temp.data.cpu().numpy() for y_temp in y_d]
        ...
        y_cc = [y_temp.data.cpu().numpy() for y_temp in y_cc]
...
def val(val_image_dir_distance, val_image_dir_crowd_count, model, epoch):
    ...
    with torch.no_grad():
        for i, (imgs, img_name) in enumerate(val_loader):
            ...
            distance_and_count = [dc.data.cpu().numpy() for dc in distance_and_count]
            ...
            for m in range(11):
                for n in range(m + 1, 11, 1):
                    if labels[m] <= anchor and labels[n] <= anchor and (labels[m] - labels[n]) * (
                            distance_and_count[m] - distance_and_count[n]) > 0:
                        count_d += 1
                    elif labels[m] > anchor and labels[n] > anchor and (labels[m] - labels[n]) * (
                            distance_and_count[m] - distance_and_count[n]) > 0:
                        count_cc += 1

            if i % 100 == 0:
                for m in range(len(distance_and_count)):
                    print('distances_and_crowd_counts: %.6f, labels: %d' % (distance_and_count[m], labels[m]))
                print('\n')
                for m in range(len(distance_and_count)):
                    print('distances_and_crowd_counts: %.6f, labels: %d' % (distance_and_count[m], labels[m]), file=terminal_file)
                print('\n', file=terminal_file)
